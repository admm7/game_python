...
laser_image = pygame.transform.scale(laser_image, (60, 200))  # Redimensionner si nécessaire
laser_image2 = pygame.transform.scale(laser_image2, (60, 120))
...
ship_image = pygame.image.load("ufo2.png")
ship_image = pygame.transform.scale(ship_image, (80, 50))
...
coin_image = pygame.transform.scale(coin_image, (40, 40))  # Redimensionner l'image des pièces
...
while running:
    ...
    if elapsed_time >= 10:
        ...
        for _ in range(3):
            new_coin = Coin(screen_width, random.randint(0, screen_height - 20))
            coins.add(new_coin)
            all_sprites.add(new_coin)

        # Génération aléatoire des zaps
        for _ in range(2):
            new_zap = Zap(screen_width, random.randint(0, screen_height - 20))
            zaps.add(new_zap)
            all_sprites.add(new_zap)

        # Génération aléatoire des rockets
        for _ in range(2):
            new_rocket = Rocket(screen_width, random.randint(0, screen_height - 20))
    ...
    # Détection de collision entre le joueur et les zaps
    zap_hit_list = pygame.sprite.spritecollide(player, zaps, False)
    if zap_hit_list:
    ...
    # Détection de collision entre le joueur et les rockets
    rocket_hit_list = pygame.sprite.spritecollide(player, rockets, False)
    if rocket_hit_list:
        player.electrocute()  # Déclencher l'animation d'électrocution
        player.rect.centerx = 100  # Réinitialiser la position horizontale du joueur
        player.rect.centery = screen_height // 2  # Réinitialiser la position verticale du joueur
